...
exports.generate = async (req, res, next) => {
  try {
    ...
    const [{ count }] = await knex("queues")
      .count("uuid", { as: "count" })
      .whereRaw(
        "DATE(created_at) = DATE(current_timestamp()) AND window_id = ?",
        [window_id]
      );
    ...
    const [queue] = await knex.select("*").from("queues").where({ uuid });
...
exports.read = async (req, res, next) => {
  try {
    const { window_id = null, get_queues = false } = req.query;

    let where = `DATE(created_at) = DATE(current_timestamp()) AND status = 'pending' ${
      Boolean(window_id) ? ` AND window_id = '${window_id}'` : ""
    }`;
    ...
    if (get_queues) {
      ...
      if (windows.length > 0) {
        queues = await Promise.all(
          windows.map(async (w) => {
            const [queue] = await knex.select("*").from("queues").whereRaw(`
...
exports.updateSpecific = async (req, res, next) => {
  try {
    ...
    const queues = await knex("queues").where("uuid", id).update({
      status: "completed",
      time_elapsed,
    });
...
exports.history = async (req, res, next) => {
  try {
    ...
    const [window] = await knex.select("*").table("windows").where({
      teller_id: id,
    });
...
exports.windowHistory = async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    if (!window) {
      return res.json({
        message: "Successfully retrieved window history.",
        sub: [],
      });
    }
